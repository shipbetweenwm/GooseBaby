...
import '../../skills/cua_brain.dart';
import '../../skills/cua_som.dart';
...
class _ChatPanelState extends State<ChatPanel> with SingleTickerProviderStateMixin {
  ...
  String? get _cuaLastAction => _cs.cuaLastAction;
  set _cuaLastAction(String? v) => _cs.cuaLastAction = v;
  String? get _cuaLastGoal => _cs.cuaLastGoal;
  set _cuaLastGoal(String? v) => _cs.cuaLastGoal = v;
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      // ── CUA 模式：Actor 角色 + Brain-Actor 协作 ──
      if (_agentMode == AgentMode.cua) {
        systemPrompt += '''

## CUA 操作模式（Brain-Actor 架构）
你当前处于 CUA（Computer Use Agent）的 Actor 角色，唯一可用的工具是 `cua` 和 `think`。
- Brain（视觉模型）负责看截图、判断操作结果、决定下一步目标
- 你（Actor）负责根据 Brain 的目标指令，调用 cua 工具执行具体操作

### 坐标系统
所有坐标归一化到 0~1000。左上角 (0,0)，右下角 (1000,1000)。

### 核心规则（必须严格遵守）
1. **必须调用 cua 工具**：所有桌面操作都必须通过 `cua(action=...)` 工具执行，**绝对禁止只用文字描述操作步骤而不实际调用工具**。
2. **follow Brain 的 next_goal**：每次收到「🧠 Brain 分析结果」后，严格按照其中的「下一步目标」调用 cua 工具执行。
3. **think 的正确用法**：`think` 只用于规划思路，think 之后必须紧接着调用 `cua` 工具执行操作。绝不能 think 后直接输出文字回复。
4. **持续操作循环**：cua 操作 → Brain 分析截图 → Brain 判断成功/失败 → cua 下一步。重复直到任务完成。
5. **失败处理**：当 Brain 判断上一步 Failed 时，根据 Brain 的 next_goal 换方法重试，不要放弃。
6. **任务完成标准**：只有当屏幕上实际看到预期结果时（Brain 确认 Success 且目标已达成），才停止调用工具并输出最终简短总结。

### 操作后自动截图
- mouse_click/key_type/key_combo/open_app/mouse_scroll/mouse_drag 操作后会自动截图
- 截图由 Brain 分析，你只需要等待 Brain 的分析结果，然后执行下一步
...
状态: Success
分析: 当前页面显示了搜索结果...
下一步目标: 点击第一个搜索结果 (600, 200)
```
你必须按照「下一步目标」调用 cua 工具。''';
      }
      ...
      if (isCUA || response!.hasToolCalls) {
        ...
        // ── CUA 多模态模式 ──
        // CUA 必须用支持 function calling 的主模型（视觉模型大多不支持 tools）
        // 截图分析由视觉模型单独完成（通过 analyzeScreenshot 回调）
        final isCUA = _agentMode == AgentMode.cua;
        ...
        final loopResult = await AgentLoop.run(
          ...
          mode: _agentMode,
          userRequest: text,
          analyzeScreenshot: isCUA ? _buildScreenshotAnalyzer(llmManager) : null,
...
请执行当前步骤，直接给出结果。''';
  ...
  /// 构建 CUA Brain 回调（参考 TuriX-CUA 的 Brain-Actor 架构）
  ///
  /// Brain（视觉模型）负责：看截图 → 判断上步成功/失败 → 输出 next_goal
  /// Actor（主模型）负责：根据 Brain 的 next_goal → 调用 cua 工具执行
  Future<String?> Function(String, String, Map<String, dynamic>) _buildScreenshotAnalyzer(
    LLMManager llmManager,
  ) {
    return (String base64Image, String mimeType, Map<String, dynamic> screenInfo) async {
      final imgW = screenInfo['width'] as int? ?? 1728;
      final imgH = screenInfo['height'] as int? ?? 1117;
      final imageType = screenInfo['imageType'] as String? ?? 'screenshot';

      // 读取 SOM 标记数据（由 _drawSomOnPng 写入）
      final somMarkers = CuaSom.lastMarkers;
      final hasSom = somMarkers.isNotEmpty;

      final brainSystemPrompt = CuaBrainPrompts.buildSystemPrompt(
        currentTask: _cuaLastGoal ?? 'CUA 桌面操作',
        somMarkerCount: somMarkers.length,
        somMarkerList: hasSom ? CuaSom.buildMarkerListText(somMarkers) : '',
      );

      // 构建 Brain 的 user message
      final brainUserMsg = imageType == 'confirm_screenshot'
          ? CuaBrainPrompts.buildCompareUserMessage(
              previousAction: _cuaLastAction,
              previousGoal: _cuaLastGoal,
            )
          : CuaBrainPrompts.buildUserMessage(
              previousAction: _cuaLastAction,
              previousGoal: _cuaLastGoal,
            );

      // analyzeScreenshot 只接受 prompt 参数，将 system prompt 合并到 user prompt 中
      // SOM 模式下不需要坐标系说明（用标记号代替坐标）
      final coordSection = hasSom
          ? '## SOM 标记\n'
            '截图上的红色编号圆圈对应可交互元素，请用 [N] 引用。\n'
            'Actor 会自动将标记号转换为坐标执行操作。'
          : '## 坐标系\n'
            '所有坐标归一化到 0~1000。左上角 (0,0)，右下角 (1000,1000)。\n'
            'X 轴 0~1000 对应图片从左到右的全部宽度（图片宽 ${imgW} 像素）。\n'
            'Y 轴 0~1000 对应图片从上到下的全部高度（图片高 ${imgH} 像素）。\n'
            '你报告的所有坐标必须是 0 到 1000 之间的整数。';

      final fullPrompt = '$brainSystemPrompt\n\n$brainUserMsg\n\n$coordSection';

      // 调用视觉模型（Brain）
      final rawOutput = await llmManager.analyzeScreenshot(
      ...
      // 解析 Brain 输出
      final brain = parseBrainOutput(rawOutput);
      debugPrint('🧠 CUA Brain: evaluate=${brain.stepEvaluate}, nextGoal=${brain.nextGoal}');

      // SOM 模式下：将 [N] 标记引用解析为坐标，供 Actor 使用
      String resolvedGoal = brain.nextGoal;
      if (hasSom) {
        resolvedGoal = CuaSom.resolveMarkers(resolvedGoal, somMarkers);
        if (resolvedGoal != brain.nextGoal) {
          debugPrint('🏷️ SOM 解析: "${brain.nextGoal}" → "$resolvedGoal"');
        }
      }

      // 构建 Actor 消息：Brain 的分析结果 + 解析后的 next_goal
      final actorMessage = buildActorBrainMessage(CuaBrainResult(
        analysis: brain.analysis,
        stepEvaluate: brain.stepEvaluate,
        askHuman: brain.askHuman,
        nextGoal: resolvedGoal,
      ));
  ...
  void _switchConversation(String conversationId) {
    ...
    // 如果当前有异步流程在执行，先取消它再切换
    if (_lockedConversationId != null) {
      _stopCurrentSession();
    }
